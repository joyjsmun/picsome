...
function Image({className, img}) {
    const {toggleFavorite,addItemToCart,removeItemToCart,cartItems} = useContext(Context)
    ...
    function cartIcon(){
        ...
        if(insideCartItem){
            return hovered && <i className="ri-add-circle-fill cart" onClick={() => removeItemToCart(img.id)}></i>
        }else if(hovered){
